...
public class RippleLayout extends LinearLayout {
    ...
    private void initView(Context context) {
        mRevealPaint.setColor(getResources().getColor(R.color.lv_item_background));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawColor(getResources().getColor(R.color.ll_gray));
        super.onDraw(canvas);
        if (mStepRadius == 0) {
            return;
        }
...
//        mRadius = getMax(radiusLeftTop, radiusRightTop, radiusLeftBottom, radiusRightBottom);
    ...
    public boolean performClick() {
        ...
        postDelayed(new Runnable() {
            @Override
            public void run() {
                RippleLayout.super.performClick();
            }
        }, 150);
        return true;
