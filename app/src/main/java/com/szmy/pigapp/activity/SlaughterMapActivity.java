...
public class SlaughterMapActivity extends BaseActivity {
	/**
	 * MapView 是地图主控件
	 */
	...
	private SlaughterhouseEntry entity;
	...
	protected void onCreate(Bundle arg0) {
		...
		((TextView) findViewById(R.id.def_head_tv)).setText("屠宰场地图");
		...
		mBaiduMap.setOnMarkerClickListener(new OnMarkerClickListener() {
			...
			public boolean onMarkerClick(final Marker marker) {
				...
				if (myZoom == MyMapZoom.BIG) {
					// 获得marker中的数据
					flag = 3;
					entity = (SlaughterhouseEntry) marker.getExtraInfo().get("info");
					
					if (entity == null)
						return true;
					// 将marker所在的经纬度的信息转化成屏幕上的坐标
					if (entity.getCompanyName().equals("")) {
						mInfoWindowsHolder.tvName.setText(entity.getRealName());
					} else {
						if (entity.getCompanyName().length()>8) {
							mInfoWindowsHolder.tvName.setText(entity.getCompanyName()+"       ");
						} else {
							mInfoWindowsHolder.tvName.setText(entity.getCompanyName());
						}
					...
					mInfoWindowsHolder.tvCount.setText(entity.getNumber() + "头/日");
					mInfoWindowsHolder.tvAddress.setText("【"
							+ entity.getProvince() + "】");
					mInfoWindowsHolder.tvType.setText(entity.getBusiness());
	...
	private void info() {
		Intent intent = new Intent(SlaughterMapActivity.this,
	...
	private void addOverly(SlaughterhouseEntry info) {
		...
		if (info.getCompanyName().equals("")) {
			...
			mOverlyHolder.tvNum.setText(info.getCompanyName());
		}
