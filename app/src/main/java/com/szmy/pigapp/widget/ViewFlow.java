...
public class ViewFlow extends AdapterView<Adapter> {
	 ...
	 */
	/*
	 * public int pointToPosition(int x, int y) { Rect frame = mTouchFrame; if
	 * (frame == null) { mTouchFrame = new Rect(); frame = mTouchFrame; }
	 * 
	 * final int count = getChildCount(); for (int i = count - 1; i >= 0; i--) {
	 * final View child = getChildAt(i); if (child.getVisibility() ==
	 * View.VISIBLE) { child.getHitRect(frame); if (frame.contains(x, y)) {
	 * return i; } } } return INVALID_POSITION; }
	 */
	...
	protected void onDetachedFromWindow() {
		...
		if (mPerformClick != null) {
			...
			Log.e("--------",
					"  onDetachedFromWindow removeCallbacks(mPerformClick)");
		}
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			// Log.e("------->",
			// "ACTION_MOVE tempDeltaX:"+tempDeltaX+" tempdeltaY: "+tempdeltaY+" mTouchSlop:"+mTouchSlop+" isXMoved:"+isXMoved+" isYMoved:"+isYMoved+" isClick:"+mIsClick);
			// ---------add start
	...
	private void postViewSwitched(int direction) {
		...
		if (mIndicator != null) {
			...
			if(view!=null)
			mIndicator.onSwitched(view, mCurrentAdapterIndex);
		}
		if (mViewSwitchListener != null) {
			mViewSwitchListener
