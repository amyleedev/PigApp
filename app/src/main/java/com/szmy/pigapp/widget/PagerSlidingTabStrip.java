...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		// indicatorPadding =
		// a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_indicatorPaddingLeftRight,
		// indicatorPadding);

		indicatorPadding = (int) context.getResources().getDimension(
				R.dimen.indicator_padding_left_right);
		...
		// int width= (int)
		// getContext().getResources().getDimension(R.dimen.pager_sliding_tab_strip_width);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_CANCEL:
			if (pager != null && pager.getAdapter() != null
				...
				if (!moveRight
						&& currentPosition + 1 <= pager.getAdapter().getCount() - 1) {

					pager.setCurrentItem(currentPosition + 1);
				}
				if (moveRight && currentPosition - 1 >= 0) {
					pager.setCurrentItem(currentPosition - 1);
				}
			}
			break;
	...
	@SuppressLint("NewApi") private void updateTabStyles() {
		for (int i = 0; i < tabCount; i++) {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				TextView tab = (TextView) v;
				if (v == currentTab) {
					tab.setTextColor(getContext().getResources().getColor(
							R.color.mail_search_tab_bottom));
				} else {
					tab.setTextColor(tabTextColor);
				}
			}
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			scrollToChild(pager.getCurrentItem(), 6);
			invalidate();
